    ...
    return (
      <main className="container">
        <div className="card" style={{marginTop: 50, textAlign: 'center'}}>
          ...
          <p style={{margin: '12px 0', fontSize: 14, color: '#64748b'}}>הזן סיסמת עריכה לביצוע שינויים מיידיים באתר</p>
          <input type="password" placeholder="סיסמת מנהל" value={password} onChange={e => setPassword(e.target.value)} style={{padding: 10, width: '80%', borderRadius: 8, border: '1px solid #cbd5e1', marginBottom: 12, textAlign: 'center'}} />
          <br />
          <button onClick={() => password ? setIsAuth(true) : alert('נא להזין סיסמה')} className="btn btn-goo" style={{width: '80%'}}>כניסה למערכת</button>
        </div>
      </main>
    );
  }
